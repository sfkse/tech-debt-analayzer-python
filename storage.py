...
class SupabaseStorageService:
    ...
    def __init__(self):
        ...
        if not supabase_url or not supabase_key:
            logger.warning(
                "Supabase credentials not found in environment variables. Storage will be disabled."
            )
            logger.debug(
    ...
    def store_scan_results(self, git_url: str, results: List[Dict]) -> Optional[str]:
        ...
        try:
            ...
            if results:
                ...
                logger.debug(
                    f"Inserting {len(issues_data)} individual issues for scan {scan_id}"
                )
            ...
            logger.error(
                f"Failed to store scan results for {git_url}: {e}", exc_info=True
            )
    ...
    def _store_failed_scan_record(
        self, scan_id: str, git_url: str, error_message: str
    ) -> Optional[str]:
        ...
        try:
            ...
            logger.error(
                f"Failed to store failed scan record for {scan_id}: {e}", exc_info=True
            )
            return None
